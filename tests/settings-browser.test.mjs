...
test('settings save applies cached snapshot, remains safe, and rejects stale tab', async () => {
  ...
  try {
    ...
    await pageA.locator('#settingsJobs button').filter({ hasText: pattern }).click();
    await pageA.locator('#settingDisplayName').fill('共用名稱');
    await pageA.locator('#settingMarket').selectOption('美股');
    await pageA.locator('#settingsForm button[type=submit]').click();
    await assertEventually(async () => assert.equal(overrides[pattern]?.market, '美股'));
    await pageA.locator('#settingsJobs button').filter({ hasText: exact }).click();
    assert.equal(await pageA.locator('#settingDisplayName').inputValue(), '共用名稱');
    assert.equal(await pageA.locator('#settingMarket').inputValue(), '美股');
    ...
    await pageA.locator('#settingsForm button[type=submit]').click();
    await assertEventually(async () => assert.equal(overrides[exact]?.displayName, '特定日期'));
    assert.deepEqual(overrides[exact], { displayName: '特定日期' });
    await pageA.locator('#settingsJobs button').filter({ hasText: pattern }).click();
    await pageA.locator('#settingMarket').selectOption('台股');
    await pageA.locator('#settingsForm button[type=submit]').click();
    await assertEventually(async () => assert.equal(overrides[pattern]?.market, undefined));
    await pageA.locator('#settingsJobs button').filter({ hasText: exact }).click();
    assert.equal(await pageA.locator('#settingMarket').inputValue(), '台股');
    assert.deepEqual(overrides[exact], { displayName: '特定日期' });
    assert.equal(collectionCount, 2);
